...
class GATConv(MessagePassing):
    ...
    def forward(  # noqa: F811
        ...
        return_raw_attention_weights: bool = None,
    ) -> Union[
            Tensor,
            Tuple[Tensor, Tuple[Tensor, Tensor]],
            Tuple[Tensor, SparseTensor],
        ...
        alpha, alpha_raw = self.edge_updater(edge_index, alpha=alpha, edge_attr=edge_attr,
                                  size=size)
        ...
        if isinstance(return_raw_attention_weights, bool):
            if isinstance(edge_index, Tensor):
                if is_torch_sparse_tensor(edge_index):
                    # TODO TorchScript requires to return a tuple
                    adj = set_sparse_value(edge_index, alpha_raw)
                    return out, (adj, alpha_raw)
                else:
                    return out, (edge_index, alpha_raw)
            elif isinstance(edge_index, SparseTensor):
                return out, edge_index.set_value(alpha_raw, layout='coo')
        else:
            return out
